...
const InvestorMessagesScreen = ({ navigation }: any) => {
  const { currentUser, messages, setMessages, users, fundingPosts, blockUser, unblockUser, isBlocked } = useApp();
  ...
  const togglePlayAudio = useCallback(async (msgId: string, uri: string, _duration: number) => {
  ...
  const handleMicPress = useCallback(async () => {
    ...
    if (!isRecordingRef.current) {
      ...
      try {
        ...
        timerRef.current = setInterval(() => {
          ...
          setRecordSeconds(recordSecsRef.current);
        }, 1000);
      } catch (error) {
        Alert.alert('Recorder Error', 'Failed to start recording.');
        console.log('Investor start recording error:', error);
      }
      return;
    }
    ...
    try {
      ...
      console.log('Investor handleMicPress error:', error);
    }
  }, [activeChatUserId, currentUser, startWave, stopWave]);
  ...
  const handleMicLongPress = useCallback(async () => {
    ...
    catch (err: any) { console.warn('[InvestorMessages] startRecorder error:', err); Alert.alert('Error', 'Could not start recording. Please restart the app and try again.'); return; }
    isRecordingRef.current = true;
    setIsRecording(true);
    startWave();
    timerRef.current = setInterval(() => { recordSecsRef.current += 1; setRecordSeconds(recordSecsRef.current); }, 1000);
  }, [startWave]);
  ...
  // Farmers who have active land posts
  const farmersWithLand = Array.from(
    new Set(fundingPosts.filter(p => p.status === 'Active').map(p => p.farmerId))
  ...
  const incomingMessages = messages.filter(m => m.receiverId === currentUserId);
  ...
  const conversations = allPartnerIds.map(partnerId => {
  ...
  const openChat = async (userId: string) => {
    ...
    setMessages(prev => prev.map(m =>
      m.senderId === userId && m.receiverId === currentUserId ? { ...m, read: true } : m
    ));
    try {
      if (currentUserId) {
      ...
      console.log('Investor openChat read update error:', error);
    }
  };

  const sendMessage = async () => {
    if (!chatText.trim() || !activeChatUserId || !currentUser) return;
    // FIXED: Safe check using currentUser.id
    ...
    try {
      ...
      console.log('Investor sendMessage error:', error);
    }
  };
  ...
  const sendImageMsg = async () => {
    ...
    try {
      ...
      console.log('Investor sendImageMsg error:', error);
    }
  };
    ...
    <View style={styles.container}>
      ...
      {conversations.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <Text style={[styles.emptySubtitle, isUrdu && styles.rtlText]}>{t('tapToMessageFarmerLand')}</Text>
        </View>
      ) : (
        <FlatList
          data={conversations}
            ...
            <TouchableOpacity style={styles.convCard} onPress={() => openChat(item.partnerId)} activeOpacity={0.8}>
              {item.partner?.profilePic ? (
                ...
                <View style={styles.avatarPH}>
                  <Text style={styles.avatarInit}>{item.partner?.firstName?.[0]}{item.partner?.lastName?.[0]}</Text>
                </View>
              )}
      ...
      <Modal visible={showFarmerList} animationType="slide" transparent statusBarTranslucent onRequestClose={() => setShowFarmerList(false)}>
        <View style={styles.farmerListOverlay}>
          <View style={styles.farmerListCard}>
            ...
            {farmersWithLand.length === 0 ? (
              <Text style={[styles.noFarmersText, isUrdu && styles.rtlText]}>{t('noFarmersWithLand')}</Text>
            ) : (
              <FlatList
                data={farmersWithLand}
                  ...
                  return (
                    <TouchableOpacity style={styles.farmerListItem} onPress={() => openChat(farmer.id)} activeOpacity={0.8}>
                      {farmer.profilePic ? (
                        ...
                        <View style={styles.farmerListAvatarPH}>
      ...
      {/* PROFILE VISIT MODAL */}
      <Modal visible={showProfileModal} animationType="slide" statusBarTranslucent onRequestClose={() => { setShowProfileModal(false); setShowReviewsModal(false); setProfileImageViewerVisible(false); }}>
        <View style={styles.profileModal}>
          <View style={styles.profileModalHeader}>
            <TouchableOpacity onPress={() => { setShowProfileModal(false); setShowReviewsModal(false); setProfileImageViewerVisible(false); }} style={styles.profileModalBack}>
          ...
          {profileUser && (
            <ScrollView
              key={`investor-profile-${profileUser?.id}-${profileModalOpenCount}`}
              style={{ flex: 1 }}
            ...
            >
              <View style={styles.profileModalAvatarSection}>
                {profileUser.profilePic ? (
                  <TouchableOpacity activeOpacity={0.85} onPress={() => setProfileImageViewerVisible(true)}>
        ...
        <ImageViewer
          ...
          images={profileUser?.profilePic ? [profileUser.profilePic] : []}
          initialIndex={0}
          ...
          <View style={styles.chatHeader}>
            ...
            <TouchableOpacity onPress={() => activePartner && !isAdminUser(activePartner) && openProfileVisit(activePartner)} activeOpacity={0.8} style={styles.chatHeaderTouchable}>
              {activePartner?.profilePic ? (
                ...
                <View style={styles.chatAvatarPH}>
                  <Text style={styles.chatAvatarInit}>{activePartner?.firstName?.[0]}{activePartner?.lastName?.[0]}</Text>
                </View>
              )}
                  ...
                  <View style={[styles.bubble, isMine ? styles.bubbleMine : styles.bubbleTheirs]}>
                    {item.audio ? (
                      <TouchableOpacity
                        ...
                        onPress={() => togglePlayAudio(item.id, item.audio!, item.audioDuration ?? 0)}
                        activeOpacity={0.7}>
                      ...
                      <Text selectable style={[styles.bubbleText, isMine ? styles.bubbleTextMine : styles.bubbleTextTheirs]}>
                        {item.text}
                      </Text>
                    )}
                    <Text style={[styles.bubbleTime, isMine ? { color: '#CE93D8' } : { color: '#9E9E9E' }]}>
          ...
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
            ...
            <View style={styles.chatInputBar}>
              ...
              {chatText.trim() ? (
                ...
                <TouchableOpacity
                  style={[styles.micBtnWrap, styles.micBtn, isRecording && styles.micBtnActive]}
...
const styles = StyleSheet.create({
  ...
  newMsgBtn: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#EDE7F6', justifyContent: 'center', alignItems: 'center' },
  ...
  convCard: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', borderRadius: 18, padding: 14, marginBottom: 10, gap: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.07, shadowRadius: 8, elevation: 4 },
  avatar: { width: 54, height: 54, borderRadius: 27, borderWidth: 2, borderColor: '#6A1B9A' },
  avatarPH: { width: 54, height: 54, borderRadius: 27, backgroundColor: '#6A1B9A', justifyContent: 'center', alignItems: 'center' },
  avatarInit: { color: '#FFFFFF', fontSize: 20, fontWeight: '800' },
  convInfo: { flex: 1 },
  ...
  rolePill: { backgroundColor: '#EDE7F6', borderRadius: 20, paddingHorizontal: 8, paddingVertical: 2 },
  rolePillText: { fontSize: 11, fontWeight: '700', color: '#6A1B9A' },
  ...
  farmerListCard: { backgroundColor: '#FFFFFF', borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 20, maxHeight: '75%' },
  ...
  farmerListItem: { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F5F5F5' },
  farmerListAvatar: { width: 46, height: 46, borderRadius: 23 },
  farmerListAvatarPH: { width: 46, height: 46, borderRadius: 23, backgroundColor: '#6A1B9A', justifyContent: 'center', alignItems: 'center' },
  ...
  chatModal: { flex: 1, backgroundColor: '#F8F0FF' },
  ...
  chatAvatar: { width: 42, height: 42, borderRadius: 21, borderWidth: 2, borderColor: '#6A1B9A' },
  chatAvatarPH: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#6A1B9A', justifyContent: 'center', alignItems: 'center' },
  chatAvatarInit: { color: '#FFFFFF', fontSize: 16, fontWeight: '800' },
  chatHeaderInfo: { flex: 1 },
  chatHeaderName: { fontSize: 16, fontWeight: '800', color: '#1B1B1B' },
  chatHeaderRole: { fontSize: 12, color: '#6A1B9A', fontWeight: '600' },
  ...
  plusBtn: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#EDE7F6', justifyContent: 'center', alignItems: 'center' },
  plusBtnText: { fontSize: 22, color: '#6A1B9A', fontWeight: '700', lineHeight: 26 },
  ...
  sendBtn: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#6A1B9A', justifyContent: 'center', alignItems: 'center' },
  sendBtnDisabled: { backgroundColor: '#CE93D8' },
  sendBtnText: { color: '#FFFFFF', fontSize: 18, fontWeight: '700' },
  rtlText: { textAlign: 'right', writingDirection: 'rtl' },
  rtlInput: { textAlign: 'right' },
  recordingBar: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#FFFFFF', paddingHorizontal: 16, paddingVertical: 10, borderTopWidth: 1, borderTopColor: '#EDE7F6' },
  ...
  recordingWaveBar: { width: 3, height: 18, borderRadius: 2, backgroundColor: '#6A1B9A' },
  ...
  audioPlayBtnTheirs: { backgroundColor: '#EDE7F6' },
  audioPlayIcon: { fontSize: 14 },
  audioPlayIconMine: { color: '#FFFFFF' },
  audioPlayIconTheirs: { color: '#6A1B9A' },
  ...
  audioWaveBarTheirs: { backgroundColor: 'rgba(106,27,154,0.35)' },
  audioWaveBarMineFilled: { backgroundColor: 'rgba(255,255,255,1)' },
  audioWaveBarTheirsFilled: { backgroundColor: '#6A1B9A' },
  audioDuration: { fontSize: 11, fontWeight: '600', minWidth: 32 },
  audioDurationMine: { color: '#CE93D8' },
  ...
  profileModalSeeReviews: { fontSize: 13, color: '#6A1B9A', fontWeight: '700', textDecorationLine: 'underline', marginBottom: 12 },
...
export default InvestorMessagesScreen;
