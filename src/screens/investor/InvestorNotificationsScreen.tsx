...
const InvestorNotificationsScreen = ({ navigation }: any) => {
  ...
  const markAllRead = () => {
    ...
    if (currentUser?.id) {
      markAllNotificationsAsReadInFirebase(currentUser.id).catch(e =>
        console.warn('[InvestorNotif] markAllRead Firestore error:', e)
      );
    }
  };
  ...
  const markOneRead = (id: string) => {
    ...
    markNotificationAsReadInFirebase(id).catch(e =>
      console.warn('[InvestorNotif] markOneRead Firestore error:', e)
    );
  };
        ...
        { text: isUrdu ? 'منسوخ' : 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            deleteNotificationInFirebase(id).catch(e =>
              console.warn('[InvestorNotif] deleteNotif Firestore error:', e)
            );
          },
        },
      ],
  ...
  const getIcon = (msg: string) => {
    if (msg.includes('approved') || msg.includes('منظور') || msg.includes('Approved')) return '✅';
    if (msg.includes('review') || msg.includes('جائزہ') || msg.includes('star')) return '⭐';
    if (msg.includes('message') || msg.includes('پیغام')) return '💬';
    if (msg.includes('fund') || msg.includes('فنڈ') || msg.includes('interest') || msg.includes('دلچسپی')) return '💼';
    return '🔔';
  };
        ...
        <View style={styles.detailModal}>
          ...
          {selectedNotif && (
            <ScrollView contentContainerStyle={styles.detailContent} showsVerticalScrollIndicator={false}>
              ...
              <View style={[styles.readBadge, { backgroundColor: selectedNotif.read ? '#F3E5F5' : '#FFF8E1' }]}>
                <Text style={[styles.readBadgeText, { color: selectedNotif.read ? '#6A1B9A' : '#F57F17' }]}>
...
const styles = StyleSheet.create({
  ...
  markReadBtn: {
    backgroundColor: '#EDE7F6', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 6,
    borderWidth: 1, borderColor: '#CE93D8',
  },
  markReadText: { fontSize: 12, fontWeight: '700', color: '#6A1B9A' },
  ...
  notifCardUnread: { borderLeftWidth: 4, borderLeftColor: '#6A1B9A', backgroundColor: '#FDF8FF' },
  notifIconWrap: {
  ...
  unreadDot: { width: 10, height: 10, borderRadius: 5, backgroundColor: '#6A1B9A', marginTop: 6 },
  ...
  detailModalBackText: { fontSize: 15, color: '#6A1B9A', fontWeight: '700' },
  detailModalTitle: { fontSize: 17, fontWeight: '800', color: '#1B1B1B' },
  detailContent: { padding: 24, alignItems: 'center' },
  detailIconCircle: {
    width: 80, height: 80, borderRadius: 40, backgroundColor: '#EDE7F6',
