...
import InvestorHomeScreen from './InvestorHomeScreen';
import InvestorPostDetailScreen from './InvestorPostDetailScreen';
import InvestorMessagesScreen from './InvestorMessagesScreen';
import InvestorNotificationsScreen from './InvestorNotificationsScreen';
import InvestorComplainScreen from './InvestorComplainScreen';
import InvestorProfileScreen from './InvestorProfileScreen';
import InvestorPaymentScreen from './InvestorPaymentScreen';
import InvestorInvestmentsScreen from './InvestorInvestmentsScreen';

const { width } = Dimensions.get('window');
const Drawer = createDrawerNavigator();

const INVESTOR_SCREEN_ITEMS = [
  { name: 'InvestorHome',              labelKey: 'menuHome',                icon: '🏡', component: InvestorHomeScreen },
  { name: 'InvestorInvestments',       labelKey: 'menuMyInvestments',       icon: '💼', component: InvestorInvestmentsScreen },
  { name: 'InvestorPayment',           labelKey: 'menuPayment',             icon: '💳', component: InvestorPaymentScreen },
  { name: 'InvestorMessages',          labelKey: 'menuMessages',            icon: '💬', component: InvestorMessagesScreen },
  { name: 'InvestorNotifications',     labelKey: 'menuNotifications',       icon: '🔔', component: InvestorNotificationsScreen },
  { name: 'InvestorComplain',          labelKey: 'menuComplain',            icon: '⚠️',  component: InvestorComplainScreen },
  { name: 'InvestorProfile',           labelKey: 'menuProfile',             icon: '👤', component: InvestorProfileScreen },
];



const InvestorDrawerContent = (props: any) => {
  ...
  const investorPenalties = currentUser?.penalties || 0;
  ...
  return (
    <Animated.View style={[styles.drawerContainer, { transform: [{ translateX: slideAnim }] }]}>
        ...
        <View style={styles.drawerProfileRow}>
          ...
          <View style={styles.drawerProfileInfo}>
            ...
            <View style={styles.drawerRoleBadge}>
              <Text style={styles.drawerRoleText}>{t('drawerInvestorRole')}</Text>
            </View>
      ...
      <ScrollView style={styles.drawerMenuScroll} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
        {INVESTOR_SCREEN_ITEMS.map((item, index) => {
          const isActive = props.state.index === index;
          const badge =
            item.name === 'InvestorMessages' ? unreadMessages :
            item.name === 'InvestorNotifications' ? unreadNotifs :
            item.name === 'InvestorProfile' ? investorPenalties : 0;
          const badgeColor = item.name === 'InvestorProfile' && investorPenalties > 0 ? '#E53935' : '#E53935';
      ...
      <View style={styles.drawerFooter}>
        <View style={styles.drawerFooterLogo}>
          ...
          <Text style={styles.drawerFooterLogoText}>
            Farm <Text style={{ color: '#6A1B9A' }}>Nest</Text>
          </Text>
        </View>
...
const InvestorNavigator = () => {
  return (
    <Drawer.Navigator
      ...
      {INVESTOR_SCREEN_ITEMS.map(item => (
        <Drawer.Screen
          key={item.name}
          name={item.name}
      ...
      <Drawer.Screen
        name="InvestorPostDetail"
        component={InvestorPostDetailScreen}
...
const styles = StyleSheet.create({
  ...
  drawerAvatar: {
    width: 58, height: 58, borderRadius: 29, borderWidth: 3, borderColor: '#6A1B9A',
  },
  drawerAvatarPlaceholder: {
    width: 58, height: 58, borderRadius: 29, backgroundColor: '#6A1B9A',
  ...
  drawerRoleBadge: {
    backgroundColor: '#EDE7F6', borderRadius: 20, paddingHorizontal: 10,
    paddingVertical: 3, alignSelf: 'flex-start', marginBottom: 5,
  },
  drawerRoleText: { fontSize: 11, fontWeight: '700', color: '#6A1B9A' },
  drawerStatusBadge: {
  ...
  drawerMenuItemActive: { backgroundColor: '#EDE7F6' },
  drawerMenuIconBox: {
    width: 38, height: 38, borderRadius: 11, backgroundColor: '#F5F5F5',
    justifyContent: 'center', alignItems: 'center',
  },
  drawerMenuIconBoxActive: { backgroundColor: '#CE93D8' },
  drawerMenuIcon: { fontSize: 18 },
  drawerMenuLabel: { flex: 1, fontSize: 15, fontWeight: '600', color: '#555555' },
  drawerMenuLabelActive: { color: '#6A1B9A', fontWeight: '800' },
  drawerActiveIndicator: {
    width: 4, height: 30, borderRadius: 2, backgroundColor: '#6A1B9A',
