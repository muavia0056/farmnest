...
const BuyerNotificationsScreen = ({ navigation }: any) => {
  ...
  const showNotifModalRef = React.useRef(showNotifModal);
  ...
  const myNotifs = notifications.filter(n => n.userId === currentUser?.id).sort((a, b) => b.timestamp - a.timestamp);

  const markAllRead = () => {
    setNotifications(prev => prev.map(n => n.userId === currentUser?.id ? { ...n, read: true } : n));
    if (currentUser?.id) {
      markAllNotificationsAsReadInFirebase(currentUser.id).catch(e =>
        console.warn('[BuyerNotif] markAllRead Firestore error:', e)
      );
    }
  };
  ...
  const markOneRead = (id: string) => {
    ...
    markNotificationAsReadInFirebase(id).catch(e =>
      console.warn('[BuyerNotif] markOneRead Firestore error:', e)
    );
  };
        ...
        { text: isUrdu ? 'منسوخ' : 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            ...
            deleteNotificationInFirebase(id).catch(e =>
              console.warn('[BuyerNotif] deleteNotif Firestore error:', e)
            );
          },
        },
      ],
  ...
  const getIcon = (msg: string) =>
    msg.includes('🏆') || msg.includes('won') || msg.includes('جیت') ? '🏆' :
    msg.includes('❌') || msg.includes('did not win') || msg.includes('نہیں جیت') ? '❌' :
    msg.includes('bid') || msg.includes('بولی') ? '🏷️' :
    msg.includes('payment') || msg.includes('ادائیگی') || msg.includes('paid') || msg.includes('ادا') ? '💳' :
    msg.includes('deliver') || msg.includes('ڈیلیوری') ? '📦' :
    msg.includes('review') || msg.includes('جائزہ') || msg.includes('star') || msg.includes('ستارہ') ? '⭐' :
    msg.includes('message') || msg.includes('پیغام') ? '💬' :
    msg.includes('approved') || msg.includes('منظور') ? '✅' :
    msg.includes('penalty') || msg.includes('جرمانہ') ? '⚠️' :
    msg.includes('suspended') || msg.includes('معطل') ? '🚫' : '🔔';
  ...
  return (
    <View style={styles.container}>
      <BuyerHeader
        ...
        rightComponent={myNotifs.some(n => !n.read) ? (
      ...
      {myNotifs.length === 0 ? (
        ...
        <FlatList data={myNotifs} keyExtractor={item => item.id} contentContainerStyle={{ padding: 14 }} showsVerticalScrollIndicator={false}
          renderItem={({ item }) => {
            ...
            return (
              <TouchableOpacity
                ...
                delayLongPress={400}
                activeOpacity={0.8}>
                <View style={styles.notifIconWrap}><Text style={styles.notifIcon}>{getIcon(msg)}</Text></View>
        ...
        <View style={styles.detailModal}>
          ...
          {selectedNotif && (
            <ScrollView contentContainerStyle={styles.detailContent} showsVerticalScrollIndicator={false}>
              ...
              <View style={[styles.readBadge, { backgroundColor: selectedNotif.read ? '#E3F2FD' : '#FFF8E1' }]}>
                <Text style={[styles.readBadgeText, { color: selectedNotif.read ? '#1565C0' : '#F57F17' }]}>
...
const styles = StyleSheet.create({
  ...
  markReadBtn: { backgroundColor: '#E3F2FD', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 6, borderWidth: 1, borderColor: '#BBDEFB' },
  markReadText: { fontSize: 12, fontWeight: '700', color: '#1565C0' },
  ...
  notifCard: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: '#FFFFFF', borderRadius: 18, padding: 14, marginBottom: 10, gap: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 6, elevation: 3 },
  notifCardUnread: { borderLeftWidth: 4, borderLeftColor: '#1565C0', backgroundColor: '#FAFEFF' },
  notifIconWrap: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#E3F2FD', justifyContent: 'center', alignItems: 'center' },
  ...
  detailModalBackText: { fontSize: 15, color: '#1565C0', fontWeight: '700' },
  ...
  readBadge: { borderRadius: 20, paddingHorizontal: 18, paddingVertical: 8, marginBottom: 24 },
