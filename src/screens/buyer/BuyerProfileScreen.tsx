...
import {
  ...
  TextInput, Image, Animated, Alert, Platform, Modal,
...
const BuyerProfileScreen = ({ navigation }: any) => {
  ...
  const showPenaltyInfoRef = React.useRef(showPenaltyInfo);
  const viewerVisibleRef   = React.useRef(viewerVisible);
  React.useEffect(() => { showPenaltyInfoRef.current = showPenaltyInfo; }, [showPenaltyInfo]);
  React.useEffect(() => { viewerVisibleRef.current   = viewerVisible;   }, [viewerVisible]);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setEditMode(false); setPassword(''); setCurrentPassword(''); setShowCurrentPw(false); setShowNewPw(false); setPendingProfilePic(null);
      ...
      console.warn('[BuyerProfile] update error:', error?.code || error?.message);
    }
    };
      ...
      {text: 'Cancel', style: 'cancel'},
      {
        ...
        onPress: async () => {
          try {
            ...
            console.warn('[BuyerProfile] logout error:', error);
          }
        },
      },
    ]);
  ...
  const getInit = () => `${(currentUser?.firstName || 'U')[0]}${(currentUser?.lastName || 'U')[0]}`.toUpperCase();

  return (
    <View style={styles.container}>
      <BuyerHeader title={t('profile')} navigation={navigation} />

      {/* ── Language Toggle – top right of profile ── */}
      <View style={styles.langToggleContainer}>
        <LanguageToggle accentColor="#1565C0" />
      </View>
      ...
      <ScrollView
        ...
        <View style={styles.profileCard}>
          ...
          <View style={styles.badgeRow}>
            <View style={styles.roleBadge}><Text selectable style={styles.roleBadgeText}>{t('buyer')}</Text></View>
            <View style={[styles.statusBadge, { backgroundColor: currentUser?.accountStatus === 'Approved' ? '#E8F5E9' : '#FFF8E1' }]}>
              <View style={[styles.statusDot, { backgroundColor: currentUser?.accountStatus === 'Approved' ? '#2E7D32' : '#F9A825' }]} />
              <Text selectable style={[styles.statusText, { color: currentUser?.accountStatus === 'Approved' ? '#2E7D32' : '#F9A825' }]}>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.row}>
            ...
            <View style={{ flex: 1 }}>
              <Text selectable style={[styles.rowLabel, isUrdu && styles.rtlText]}>
                {isUrdu ? 'اکاؤنٹ نمبر (شناختی کارڈ)' : 'Account Number (CNIC)'}
              </Text>
          ...
          <View style={styles.row}>
            ...
            <View style={{ flex: 1 }}>
              ...
              {editMode ? (
                <TextInput style={[styles.editInput, isUrdu && styles.rtlInput]} value={phone} onChangeText={setPhone}
                  placeholderTextColor="#9E9E9E" keyboardType="numeric"
          ...
          <View style={styles.row}>
            ...
            <View style={{ flex: 1 }}>
              ...
              {editMode ? (
                <TextInput style={[styles.editInput, isUrdu && styles.rtlInput]} value={city} onChangeText={setCity}
        ...
        {/* Penalty Card */}
        {((currentUser?.penalties ?? 0) > 0) && (
          <View style={styles.penaltyCard}>
            <View style={styles.penaltyCardHeader}>
              <Text style={[styles.penaltyCardTitle, isUrdu && styles.rtlText]}>{t('penaltyStatus')}</Text>
            </View>
            <View style={styles.penaltyBarWrap}>
              {[1, 2, 3].map(n => (
                <View
                  ...
                  style={[
                    styles.penaltyDot,
                    (currentUser?.penalties ?? 0) >= n
                      ? styles.penaltyDotActive
                      : styles.penaltyDotInactive,
                  ]}
                />
              ))}
            </View>
            <Text selectable style={[styles.penaltyCount, isUrdu && styles.rtlText]}>
              {currentUser?.penalties}/3 {t('penaltiesReceived')}
            </Text>
            {(currentUser?.penalties ?? 0) >= 3 && (
              <Text style={[styles.penaltyWarn, isUrdu && styles.rtlText]}>
                {t('penaltyAccountSuspendWarning')}
              </Text>
            )}
            <TouchableOpacity onPress={() => setShowPenaltyInfo(true)} activeOpacity={0.8} style={{ marginTop: 10, alignSelf: 'center' }}>
              <Text style={styles.penaltyInfoLink}>{t('penaltyInfoLink')}</Text>
            </TouchableOpacity>
          </View>
        )}

        {/* Penalty Info Modal */}
        <Modal visible={showPenaltyInfo} transparent animationType="fade" onRequestClose={() => setShowPenaltyInfo(false)}>
          <View style={styles.penaltyModalOverlay}>
            <View style={styles.penaltyModalCard}>
              <Text style={[styles.penaltyModalTitle, isUrdu && styles.rtlText]}>{t('penaltySystemTitle')}</Text>
              <Text selectable style={[styles.penaltyModalBody, isUrdu && styles.rtlText]}>
                {t('penaltySystemBody')}{'\n\n'}
                {t('penaltyFirst')}{'\n'}
                {t('penaltySecond')}{'\n'}
                {t('penaltyThird')}
              </Text>
              <TouchableOpacity style={styles.penaltyModalCloseBtn} onPress={() => setShowPenaltyInfo(false)} activeOpacity={0.85}>
                <Text style={styles.penaltyModalCloseBtnText}>{t('penaltyGotIt')}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </Modal>
...
const styles = StyleSheet.create({
  ...
  toast: { position: 'absolute', top: Platform.OS === 'ios' ? 100 : 80, left: 20, right: 20, zIndex: 999, backgroundColor: '#2E7D32', borderRadius: 14, paddingVertical: 14, paddingHorizontal: 20, shadowColor: '#2E7D32', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.4, shadowRadius: 10, elevation: 12 },
  toastText: { color: '#FFFFFF', fontSize: 15, fontWeight: '700', textAlign: 'center' },
  scrollContent: { padding: 16, paddingBottom: 40 },
  profileCard: { backgroundColor: '#FFFFFF', borderRadius: 24, padding: 24, alignItems: 'center', marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.08, shadowRadius: 12, elevation: 6 },
  avatarWrap: { position: 'relative', marginBottom: 14 },
  avatar: { width: 90, height: 90, borderRadius: 45, borderWidth: 3, borderColor: '#1565C0' },
  avatarPH: { width: 90, height: 90, borderRadius: 45, backgroundColor: '#1565C0', justifyContent: 'center', alignItems: 'center', borderWidth: 3, borderColor: '#90CAF9' },
  avatarInit: { color: '#FFFFFF', fontSize: 34, fontWeight: '800' },
  avatarEditBadge: { position: 'absolute', bottom: 0, right: -4, width: 32, height: 32, borderRadius: 16, backgroundColor: '#E3F2FD', justifyContent: 'center', alignItems: 'center', borderWidth: 2, borderColor: '#FFFFFF' },
  ...
  reviewCount: { fontSize: 13, color: '#9E9E9E', fontWeight: '500', marginLeft: 4 },
  ...
  roleBadge: { backgroundColor: '#E3F2FD', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 6 },
  roleBadgeText: { fontSize: 13, fontWeight: '700', color: '#1565C0' },
  ...
  infoCard: { backgroundColor: '#FFFFFF', borderRadius: 20, padding: 18, marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.07, shadowRadius: 8, elevation: 4 },
  ...
  editBtn: { backgroundColor: '#E3F2FD', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 7, borderWidth: 1.5, borderColor: '#BBDEFB' },
  editBtnText: { fontSize: 13, fontWeight: '700', color: '#1565C0' },
  row: { flexDirection: 'row', alignItems: 'flex-start', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F5F5F5', gap: 12 },
  ...
  editInput: { backgroundColor: '#F9F9F9', borderRadius: 10, borderWidth: 1.5, borderColor: '#E8E8E8', paddingHorizontal: 12, paddingVertical: 10, fontSize: 15, color: '#1B1B1B' },
  editMulti: { height: 70, textAlignVertical: 'top', paddingTop: 10 },
  pwRow: { flexDirection: 'row', alignItems: 'center', gap: 8 },
  pwInput: { flex: 1 },
  eyeBtn: { padding: 8, justifyContent: 'center', alignItems: 'center' },
  eyeIcon: { fontSize: 18 },
  updateBtn: { backgroundColor: '#1565C0', borderRadius: 14, paddingVertical: 15, alignItems: 'center', marginTop: 20, shadowColor: '#1565C0', shadowOffset: { width: 0, height: 5 }, shadowOpacity: 0.35, shadowRadius: 10, elevation: 7 },
  updateBtnDisabled: { backgroundColor: '#90CAF9' },
  updateBtnText: { color: '#FFFFFF', fontSize: 16, fontWeight: '800' },
  cancelBtn: { backgroundColor: '#F5F5F5', borderRadius: 14, paddingVertical: 13, alignItems: 'center', marginTop: 10, borderWidth: 1.5, borderColor: '#E0E0E0' },
  cancelBtnText: { color: '#555555', fontSize: 15, fontWeight: '600' },
  reviewsCard: { backgroundColor: '#FFFFFF', borderRadius: 20, padding: 18, marginBottom: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.07, shadowRadius: 8, elevation: 4 },
  reviewsTitle: { fontSize: 16, fontWeight: '800', color: '#1B1B1B', marginBottom: 14 },
  ...
  reviewItemDetail: { fontSize: 13, color: '#555555', lineHeight: 20, marginBottom: 4 },
  reviewerName: { fontSize: 12, color: '#1565C0', fontWeight: '600', fontStyle: 'italic' },
  penaltyCard: { backgroundColor: '#FFF8E1', borderRadius: 20, padding: 18, marginBottom: 16, borderWidth: 1.5, borderColor: '#FFE082', shadowColor: '#F9A825', shadowOffset: { width: 0, height: 3 }, shadowOpacity: 0.15, shadowRadius: 8, elevation: 4 },
  penaltyCardHeader: { flexDirection: 'row', alignItems: 'center', marginBottom: 12 },
  penaltyCardTitle: { fontSize: 16, fontWeight: '800', color: '#E65100' },
  ...
  penaltyCount: { textAlign: 'center', fontSize: 14, fontWeight: '700', color: '#BF360C', marginBottom: 4 },
  penaltyWarn: { textAlign: 'center', fontSize: 13, fontWeight: '600', color: '#C62828', marginTop: 6, lineHeight: 20 },
  penaltyInfoLink: { fontSize: 13, fontWeight: '700', color: '#1565C0', textDecorationLine: 'underline', textAlign: 'center' },
  penaltyModalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.6)', justifyContent: 'center', alignItems: 'center', padding: 30 },
  penaltyModalCard: { backgroundColor: '#FFFFFF', borderRadius: 24, padding: 28, width: '100%', shadowColor: '#000', shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.3, shadowRadius: 20, elevation: 15 },
  penaltyModalTitle: { fontSize: 20, fontWeight: '800', color: '#1B1B1B', marginBottom: 16 },
  penaltyModalBody: { fontSize: 14, color: '#444444', lineHeight: 24, marginBottom: 24 },
  penaltyModalCloseBtn: { backgroundColor: '#1A1A2E', borderRadius: 14, paddingVertical: 14, alignItems: 'center' },
  penaltyModalCloseBtnText: { color: '#FFFFFF', fontSize: 15, fontWeight: '800' },
  logoutBtn: { backgroundColor: '#FFEBEE', borderRadius: 14, paddingVertical: 15, alignItems: 'center', borderWidth: 1.5, borderColor: '#EF9A9A' },
